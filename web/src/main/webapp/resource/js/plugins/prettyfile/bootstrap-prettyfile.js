    ...
    $.fn.extend({
        prettyFile: function (options) {
            ...
            function make_form($el, text) {
                ...
                $el.hide();
                $el.after('\
				<div onclick="uploadClick(this)" class="input-append input-group"">\
					<span class="input-group-btn">\
						<button class="btn btn-white" type="button">' + text + '</button>\
					</span>\
					<input class="input-large form-control" name="' + $el.attr("name") + '_hd" type="text">\
				</div>\
				');
                $el.attr("onchange", 'uploadChange(this)');
                return $el.parent();
            };
...
function uploadChange(obj) {
    var path = $(obj).val().split('\\');
    $(obj).parent().find(".input-append input").val(path[path.length - 1]);
}
